...
int bgrep(void* hProcess, void* address, int size, unsigned char* bytes, int bytelen) {
    ...
    for (int i=0; i < size; i+=incNum) {
        ...
        int mask = 0;
        if (is512 == 1) {
        __m512i zmm = _mm512_load_si512(in+i);
        __m512i zmm2 = _mm512_load_si512(in+i+64);
        ...
        __m128i mm = _mm_loadu_si128((__m128i*)(in+i));
        __m128i cmp = _mm_cmpeq_epi8(key, mm);
        mask = _mm_movemask_epi8(cmp);
        }

        if (mask != 00) {

            for (int p=0; p < incNum; p++) {

            if (in[i+p] != bytes[0]) continue;
